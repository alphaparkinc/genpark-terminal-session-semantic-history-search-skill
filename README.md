# genpark-terminal-session-semantic-history-search-skill
...
**genpark-terminal-session-semantic-history-search-skill** is a deterministic, zero-dependency Python skill engineered for autonomous developer workflows, keyboard-first command routing, sprint velocity forecasting, and calendar conflict resolution.

> **Executive Capability**: Shell session history semantic searcher matching intents to CLI commands (Warp)
...
result = client.search_cli_history()
print(result)
```
...
```json
{
  "mcpServers": {
    "genpark-terminal-session-semantic-history-search-skill": {
      "command": "python",
      "args": ["/path/to/genpark-terminal-session-semantic-history-search-skill/mcp_server.py"]
    }
  }
}
```
